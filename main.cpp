...
public:
	...
	void InitiateMove(Vector3 newTargetPosition){
		if (!isMoving){
...
int main() {
	...
	AllEntities.push_back(std::make_unique<GameEntity>(Vector3{0.0f, 1.0f, 0.0f}, MAGENTA));
	AllEntities.push_back(std::make_unique<GameEntity>(Vector3{5.0f, 1.0f, 0.0f}, BLUE));
	...
	while (!WindowShouldClose()){
		...
		if (currentlySelected != nullptr){
			...
			if (collision.hit){
				...
				Vector3 newTargetPosition = {collision.point.x, currentlySelected->position.y, collision.point.z};
